...
export const updateShuttleStatus = (req, res, next) => {
  ...
  console.log("gpsLoc to activate>>>>>>>>>>", JSON.stringify(req.body));
  const tripGpsLoc = req.body.gpsLoc;
...
export const regenerateAccessCode = async (req, res, next) => {
  ...
  console.log("userTypes >>>>>>>", user.userType, USER_TYPE_ADMIN);

  if(!((user.userType == USER_TYPE_ADMIN) || (user.userType == USER_TYPE_SUPER_ADMIN))) {
    let err = new APIError('Unauthorized', httpStatus.UNAUTHORIZED);
    return next(err);
  }

  try {
    const userToUpdate = await User.findOneAsync({_id: userId});

    if(!userToUpdate) {
      returnObj.message = "User not found";
      return res.send(returnObj);
    } else if (userToUpdate.isDeleted) {
      returnObj.message = "User was deleted";
      return res.send(returnObj);
    }

    const newAccessCode = Utilities.generateAccessCode();
    userToUpdate.accessCode = newAccessCode;
    userToUpdate.accessCodeVerified = false;
    const savedUser = userToUpdate.saveAsync();
    if(!savedUser) {
    ...
    if(userToUpdate && (userToUpdate.userType == USER_TYPE_DRIVER)) {
      let activeTrip = await tripSchema.findOneAsync({"driver._id": userToUpdate._id, activeStatus: true});
      if(activeTrip) {
        const deactivateResult = await deactivateTripAsync(activeTrip._id);
...
function deactivateTripAsync(tripId){
  ...
  return new Promise( async (resolve, reject) => {
    ...
    try {
      const trip = await tripSchema.findOneAsync({_id:tripId});
      if(!trip) {
        returnObj.message = "Trip not found";
        return returnObj;
      }
      const driverId = trip.driver && trip.driver._id;
      const shuttleId = trip.shuttleId;

      let promises = [
        User.findOneAsync({ _id: driverId, isDeleted: false}, {password:0}),
        AdminVehicleSchema.findOneAsync({_id: shuttleId})
      ];
      ...
      if (result && result.length) {
        ...
        console.log("userAdminResult>>>>>>>>", JSON.stringify(result));
        if (!driver) {
          returnObj.message = "Driver not found";
          return resolve(returnObj);
        } else if (!vehicle) {
          ...
          if(driver.route) {
            ...
            const acceptedRequests = await tripRequestSchema.aggregateAsync(pipelineStages);

            if(acceptedRequests && acceptedRequests.length) {
              const response = await nearByShuttleAsync(tripId, {seats: acceptedRequests[0].seats})
              if(!response.success) {
                ...
                const updatedReqs = await Promise.all(updateRequestsAsync)
                ...
                if(transferToShuttle) {
                  if(trip.driver.tripType == TRIP_DYNAMIC) {
                    ...
                    transferRequestsAsync(tripId, transferToShuttle._id)
                    .then(result=>{
                      tripSchema.findOneAndUpdateAsync(
                        { _id: tripId, activeStatus: true },
                        { $set: {
